...
static void lv_min_spinbox_increment_event_cb(lv_event_t *event)
{
  ...
  if(code == LV_EVENT_SHORT_CLICKED || code  == LV_EVENT_LONG_PRESSED_REPEAT) {
    ...
    communication.sendEvent("setMinSpeed", (String)lv_spinbox_get_value(minSpeedSpinbox));
  }
}
...
static void lv_temp_spinbox_decrement_event_cb(lv_event_t *event)
{
  lv_event_code_t code = lv_event_get_code(event);
  if(code == LV_EVENT_SHORT_CLICKED || code == LV_EVENT_LONG_PRESSED_REPEAT) {
    lv_spinbox_decrement(waterTempSpinbox);

    communication.sendEvent("setTemp", (String)lv_spinbox_get_value(waterTempSpinbox));
  }
}
...
static void lv_max_spinbox_increment_event_cb(lv_event_t *event)
{
  ...
  if(code == LV_EVENT_SHORT_CLICKED || code  == LV_EVENT_LONG_PRESSED_REPEAT) {
    ...
    communication.sendEvent("setMaxSpeed", (String)lv_spinbox_get_value(maxSpeedSpinbox));
  }
}
...
static void lv_temp_spinbox_increment_event_cb(lv_event_t *event)
{
  lv_event_code_t code = lv_event_get_code(event);
  if(code == LV_EVENT_SHORT_CLICKED || code == LV_EVENT_LONG_PRESSED_REPEAT) {
    lv_spinbox_increment(waterTempSpinbox);
...
static void event_handler(lv_event_t *event)
{
  ...
  if(code == LV_EVENT_VALUE_CHANGED) {
    ...
    communication.sendEvent("setPolymer", String(buf));
  }
...
void build_settingsTab(lv_obj_t *parent) {
  ...
  lv_obj_t *waterTempSpinboxLabel = lv_label_create(settingsParent);
  lv_obj_set_style_text_color(waterTempSpinboxLabel, lv_color_hex(0xFFFFFF), 0);
  lv_obj_align_to(waterTempSpinboxLabel, polymerDropdownLabel, LV_ALIGN_OUT_BOTTOM_LEFT, 0, lv_obj_get_height(polymerDropdown));
  lv_label_set_text(waterTempSpinboxLabel, "Temperatura");

  waterTempSpinbox = lv_spinbox_create(settingsParent);
  lv_spinbox_set_range(waterTempSpinbox, 0, 30);
  lv_spinbox_set_digit_format(waterTempSpinbox, 2, 0);
  lv_spinbox_set_step(waterTempSpinbox, 1);
  lv_spinbox_step_prev(waterTempSpinbox);
  lv_obj_set_width(waterTempSpinbox, 65);
  lv_obj_align_to(waterTempSpinbox, waterTempSpinboxLabel, LV_ALIGN_OUT_RIGHT_MID, lv_obj_get_height(waterTempSpinbox) + 210, 0);

  lv_obj_t *waterTempSpinboxPlusBtn = lv_btn_create(settingsParent);
  lv_obj_set_size(waterTempSpinboxPlusBtn, lv_obj_get_height(waterTempSpinbox), lv_obj_get_height(waterTempSpinbox));
  lv_obj_align_to(waterTempSpinboxPlusBtn, waterTempSpinbox, LV_ALIGN_OUT_RIGHT_MID, 5, 0);
  lv_obj_set_style_bg_img_src(waterTempSpinboxPlusBtn, LV_SYMBOL_PLUS, 0);
  lv_obj_set_style_bg_color(waterTempSpinboxPlusBtn, lv_palette_main(LV_PALETTE_ORANGE), 0);
  lv_obj_add_event_cb(waterTempSpinboxPlusBtn, lv_temp_spinbox_increment_event_cb, LV_EVENT_ALL,  NULL);

  lv_obj_t *waterTempSpinboxMinusBtn = lv_btn_create(settingsParent);
  lv_obj_set_size(waterTempSpinboxMinusBtn, lv_obj_get_height(waterTempSpinbox), lv_obj_get_height(waterTempSpinbox));
  lv_obj_align_to(waterTempSpinboxMinusBtn, waterTempSpinbox, LV_ALIGN_OUT_LEFT_MID, -5, 0);
  lv_obj_set_style_bg_img_src(waterTempSpinboxMinusBtn, LV_SYMBOL_MINUS, 0);
  lv_obj_set_style_bg_color(waterTempSpinboxMinusBtn, lv_palette_main(LV_PALETTE_ORANGE), 0);
  lv_obj_add_event_cb(waterTempSpinboxMinusBtn, lv_temp_spinbox_decrement_event_cb, LV_EVENT_ALL, NULL);
