...
static void reset_measure_btn_cb(lv_event_t *event) {
  if (event->code == LV_EVENT_PRESSED) {
    lv_obj_set_style_text_color(lv_obj_get_child(event->target, 0), lv_palette_main(LV_PALETTE_ORANGE), 0);
    lv_obj_set_style_bg_color(event->target, lv_palette_main(LV_PALETTE_GREY), 0);
    lv_obj_set_style_bg_opa(event->target, LV_OPA_COVER, 0);
  } else if (event->code == LV_EVENT_RELEASED) {
    lv_obj_set_style_text_color(lv_obj_get_child(event->target, 0), lv_color_hex(0xFFFFFF), 0);
    lv_obj_set_style_bg_opa(event->target, LV_OPA_TRANSP, 0);
...
void build_homeTab(lv_obj_t *parent) {
  ...
  lv_obj_set_size(measuringParent, lv_obj_get_width(parent) - 155, 125);
