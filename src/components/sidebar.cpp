...
static void open_home_tab(lv_event_t *event) {
  lv_obj_t *label = lv_obj_get_child(event->target, 0);
...
void build_sidebar() {
    ...
    //Tabs
    tabview = lv_tabview_create(lv_scr_act(), LV_DIR_LEFT, SIDEBAR_WIDTH);
    ...
    lv_label_set_text(listBtnLabel, LV_SYMBOL_LIST);
